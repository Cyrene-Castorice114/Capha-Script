...
check_package(){
    ...
    if $package_category install proot-distro -y >/dev/null 2>&1; then
...
uv_install(){
    ...
    if [ -f "$HOME/.local/bin/uv" ];then
        ...
        curl -LsSf "https://gitcode.com/HA-Hoshino_Ai/capha_cyrene/-/raw/master/uv-installer-for-astrbot-termux.sh" | sh
    fi'
...
astrbot_download(){
    ...
    #切换uv的下载方式为复制
    echo -e "$blue 正在安装指定稳定版本$green astrbot 4.22.3 $color版本(astrbot==4.22.3)"
    proot-distro login astrbot -- bash -c 'echo "export UV_LINK_MODE=copy" >> $HOME/.bashrc && $HOME/.local/bin/uv tool install astrbot==4.22.3'
