...
check_dir(){
    ...
    do
        if [ -d "$HOME/.capha/acg" ];then
            echo -ne "$grey检测到PATH目录:$HOME/.capha/acg   Path directory detected: $HOME/.capha/acg"
            chmod 777 $HOME/.capha/*
            sleep 0.5
            break
        else
            echo -ne "$grey未检测到PATH目录，开始创建...   Path directory not detected, start creating..."
            mkdir $HOME/.capha/acg
        fi 
    done
}
